...
namespace Fnt2TMPro.EditorUtilities
{
    ...
    public class FntParse
    {
        ...
        public static FntParse GetFntParse(ref string text)
        {
            ...
            {
                Debug.LogError("Empty .fnt content passed to parser.");
                return null;
            }
            FntParse parse = null;
        ...
        public void DoXMLPase(ref string content)
        {
            ...
            {
                Debug.LogError($"Failed to load XML .fnt: {ex.Message}");
                return;
            }
        ...
        public void DoTextParse(ref string content)
        {
            ...
            if (lines.Length < 2)
            {
                Debug.LogError("Invalid text .fnt format: not enough lines.");
                return;
            }
            ...
            for (int j = 0; j < textureNames.Length; j++)
            {
                if (j + 2 >= lines.Length)
                {
                    Debug.LogError("Invalid text .fnt format: missing page lines.");
                    return;
                }
